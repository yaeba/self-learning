...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def inGroup(s: String, group: List[String]): Boolean = group.exists(s.startsWith)

    val primaryCols = columnNames.filter(inGroup(_, primary)).map(col)
    val workingCols = columnNames.filter(inGroup(_, working)).map(col)
    val otherCols = columnNames.filter(x => inGroup(x, other) && !inGroup(x, primary ++ working)).map(col)
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    val grouping: List[Column] = List(col("working"), col("sex"), col("age"))

    summed.groupBy(grouping: _*)
      .agg(
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map{
      row => TimeUsageRow(row.getAs[String](0), row.getAs[String](1), row.getAs[String](2),
        row.getAs[Double](3), row.getAs[Double](4), row.getAs[Double](5))
    }
