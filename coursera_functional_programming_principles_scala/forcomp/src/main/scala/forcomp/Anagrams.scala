...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toList.groupBy(_.toLower).map(t => (t._1, t._2.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case (char, freq) :: xs =>
      (for (i <- (1 to freq).toList; ps <- combinations(xs)) yield (char, i) :: ps) ++ combinations(xs)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagramRecursive(occurrence: Occurrences): List[Sentence] =
      if (occurrence.isEmpty) List(Nil)
        ...
        for {
          ...
          remaining <- anagramRecursive(subtract(occurrence, poss))
