...
abstract class TweetSet extends TweetSetInterface {
  ...
  // Helper recursive function for mostRetweeted
  def mostRecursive(tweet: Tweet): Tweet
  /**
...
class Empty extends TweetSet {
  ...
  // Empty union with anything
  def union(that: TweetSet): TweetSet = that

  // Empty has no retweets
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  // Traverse to leftmost, gradually moving up and do right
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    right.filterAcc(p,
      left.filterAcc(p,
  ...
  // Union leftmost elem with "that" first
  def union(that: TweetSet): TweetSet = right.union(left.union(that.incl(elem)))

  def mostRetweeted: Tweet = this.mostRecursive(elem)

  def mostRecursive(tweet: Tweet): Tweet = {
    right.mostRecursive(
      left.mostRecursive(
        if (tweet.retweets > elem.retweets) tweet else elem
      )
    )
  }
