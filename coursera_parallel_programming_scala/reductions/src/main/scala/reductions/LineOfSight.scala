...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit =
    input.zipWithIndex.foreach {
      case (_, 0) => output(0) = 0
      case (ii, idx) => output(idx) = Math.max(ii / idx, output(idx - 1))
    }

  /** Traverses the specified part of the array and returns the maximum angle.
    */
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float =
    (from until until).foldLeft(0.0f)((acc, i) => Math.max(input(i) / i, acc))
  ...
  /** Traverses the part of the array starting at `from` and until `end`, and
    ...
    */
  def upsweep(input: Array[Float],
              from: Int,
              end: Int,
              threshold: Int): Tree = {
    val step = end - from
    if (step <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
    else {
      val mid = from + step / 2
